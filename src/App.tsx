  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-5 py-[15px]">
      <div className={`w-full ${cardMaxWidth} z-10`}>
        ...
        >
          <div className="px-5 md:px-6 py-[15px]">
            <div className={`flex items-center justify-center ${isDashboard ? 'mb-0' : 'mb-6'}`}>
              <div
                className={`line-anim left h-px bg-white/35 flex-1 relative overflow-hidden ${isSetPassword || isDashboard ? 'green' : ''
                  }`}
              >
                <span />
              </div>
              {mode === 'security' ? (
                <div className="flex items-center gap-0 shrink-0">
                  ...
                  <div className="border-2 border-white/60 rounded-full p-5 backdrop-blur-sm">
                    <HeaderIcon
                      className={`w-16 h-16 shrink-0 avatar-beat ${isSetPassword || isDashboard ? 'text-emerald-400' : 'text-rose-500'
                        }`}
                    />
                  </div>
                </div>
              ...
              <div
                className={`line-anim right h-px bg-white/35 flex-1 relative overflow-hidden ${isSetPassword || isDashboard ? 'green' : ''
                  }`}
              >
                <span />
              </div>
            ...
            {mode === 'login' && (
              <form onSubmit={handleSubmit} className="space-y-6">
                <div
                  className={`flex shadow-lg rounded-md overflow-hidden transition-transform duration-200 group border border-rose-200/40 bg-gradient-to-r from-white/95 via-white/90 to-white/80 ${isLoading ? 'opacity-70' : 'hover:scale-[1.01]'
                    }`}
                >
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className={`w-full bg-gradient-to-r from-rose-500 to-pink-500 text-white font-bold py-3 rounded-lg shadow-lg transform transition-all duration-200 tracking-widest text-sm flex items-center justify-center ${isLoading
                ...
                {feedback.message && (
                  <div
                    className={`flex items-center justify-center gap-3 rounded-lg px-4 py-3 text-sm border ${feedback.type === 'success'
                ...
                <div className="flex items-center justify-between text-white/90 text-sm mt-10">
                  <label
                    className={`flex items-center cursor-pointer group ${isLoading ? 'pointer-events-none opacity-70' : ''
                      }`}
                  >
                    <div className="relative">
                      ...
                      />
                      <div
                        className={`w-5 h-5 border-2 border-white/60 rounded transition-colors duration-200 flex items-center justify-center ${rememberMe ? 'bg-rose-500 border-rose-500' : 'group-hover:border-white bg-transparent'
                          }`}
                      >
                  ...
                  <div className="relative group">
                    <a
                      href="#"
                      className={`font-normal italic hover:text-white hover:underline transition-colors opacity-90 hover:opacity-100 ${isLoading ? 'pointer-events-none opacity-50' : ''
                        }`}
                    >
                      Esqueceu a Senha?
                    </a>
              ...
              <form onSubmit={handleSetPassword} className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="submit"
                    disabled={isLoading}
                    className={`flex-1 bg-gradient-to-r from-emerald-500 to-green-500 text-white font-bold py-3 rounded-lg shadow-lg transform transition-all duration-200 tracking-widest text-sm flex items-center justify-center ${isLoading
                        ? 'opacity-80 cursor-wait'
              ...
              <div className="space-y-1 -mt-10">
                {securityCard?.accent === 'table_load' ? (
                  ...
                  <Operations
                    ...
                    supabaseUrl={supabaseUrl}
                    supabaseKey={supabaseKey}
                    operationsPermissions={currentUser?.operations}
                  />
                ) : (
                  <Security
                    onBack={() => setMode('dashboard')}
        ...
        <p className="text-center text-red text-xs mt-1 py-4">&copy; rhControle2 - Version 2025.1 Development Benjamim Gomes & Yago Filho</p>
      </div>
    </div>
  )
}
